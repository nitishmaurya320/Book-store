...
const Navbar = ({isModalOpen,setIsModalOpen}) => {
  ...
  return (
    <div >
      <nav className={` dark:bg-gray-900 border   bg-white fixed w-full z-20 top-0 start-0 border-b ${isscrolled ? "shadow-md":""}`}>
  ...
  <div onClick={() => setDarkMode(!darkMode) } className=" md:order-4 border">
  <MdOutlineDarkMode  className='text-2xl'/>
  </div>
  <div className="flex md:order-5 space-x-3 md:space-x-0 rtl:space-x-reverse">
      <button type="button" onClick={()=>{setIsModalOpen(true)}}  className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Login</button>
  ...
  <div  className={`items-center justify-between w-full md:flex space-x- l border md:w-auto md:order-2 ${
    isopen ? "block" : "hidden"
  }`} id="navbar-sticky">
